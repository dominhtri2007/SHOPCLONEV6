...
$listAccount = $CMSNT->get_list("SELECT * FROM `accounts` WHERE $where ORDER BY id DESC LIMIT $from,$sotin1trang  ");
...
<div class="content-wrapper">
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <a type="button" href="<?=base_url('admin/accounts/'.check_string($_GET['id']));?>" class="btn btn-danger btn-block mb-5">QUAY LẠI</a>
             
                <div class="col-12 col-sm-6 col-md-3">
                    <div class="info-box mb-3">
                        <span class="info-box-icon bg-success elevation-1"><i class="fa-solid fa-cart-shopping"></i></span>
                        <div class="info-box-content">
                            <span class="info-box-text">Tài khoản đang bán LIVE</span>
                            <span class="info-box-number"><?=format_cash($CMSNT->get_row("SELECT COUNT(id)  FROM `accounts` WHERE `product_id` = '".$row['id']."' AND `buyer` IS NULL AND `status` = 'LIVE' ")['COUNT(id)']);?></span>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-md-3">
                    <div class="info-box mb-3">
                        <span class="info-box-icon bg-danger elevation-1"><i class="fa-solid fa-cart-shopping"></i></span>
                        <div class="info-box-content">
                            <span class="info-box-text">Tài khoản đang bán DIE</span>
                            <span class="info-box-number"><?=format_cash($CMSNT->get_row("SELECT COUNT(id)  FROM `accounts` WHERE `product_id` = '".$row['id']."' AND `buyer` IS NULL AND `status` = 'DIE' ")['COUNT(id)']);?></span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                                    ...
                                    <form action="" name="formSearch" method="GET">
                                        ...
                                        <div class="row">
                                            ...
                                            <select class="form-control select2bs4 col-sm-2 mb-2" name="status">
                                                ...
                                                <option <?=$status == 'DIE' ? 'selected' : '';?> value="DIE">DIE</option>
                                            </select>    
                            ...
                            <div class="row">
                               ...
                               <div class="col-sm-12 col-md-7">
                                ...
                                if ($total > $sotin1trang){echo '<center>' . pagination(base_url("index.php?module=admin&action=account-view&id=$id&status=$status&account=$account&"), $from, $total, $sotin1trang) . '</center>';}?>
                                </div>
                                <div class="col-sm-6">
                                    <button class="btn btn-danger btn-sm" type="button"
                                        onclick="deleteConfirm()" name="btn_delete"><i
                                            class="fas fa-trash mr-1"></i>Xoá đã chọn</button>
                                </div>
